...
import multiprocessing
import threading 
# Make sure these imports are at the top of base.py

# Remove the global _manager and get_manager() function entirely
# from the file. We will define the manager getter inside the class.

class EmbeddingCache:
    """A multiprocessing-safe global cache for storing embeddings."""
    
    # Static variable to hold the Manager instance and cache dictionary
    ...
    def _initialize_cache(cls):
        """Initializes the Manager and the shared dictionary once."""
        if cls.__manager_instance is None:
            # We initialize here, only when a method is first called.
            # This is NOT executed on import.
            print("--- INFO: Starting Multiprocessing Manager for EmbeddingCache ---")
            cls.__manager_instance = multiprocessing.Manager()
            cls.__cache_dict = cls.__manager_instance.dict()
            
    @classmethod
    def _get_cache(cls):
        """Gets the shared dictionary, initializing it if necessary."""
        # Ensure initialization runs only once before returning the cache
    ...
    def get(cls, content):
        ...
        # All methods must now call _get_cache() instead of accessing cls._cache directly
        return cls._get_cache().get(content)

    @classmethod
    def set(cls, content, embedding):
        """Store an embedding in the cache."""
        with cls._lock:
            cls._get_cache()[content] = embedding

    @classmethod
    def contains(cls, content):
        """Check if the embedding exists in cache."""
        return content in cls._get_cache()
